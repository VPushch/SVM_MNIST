...
class SVMDigits:
    ...
    data_dir : текстовый файл с построчно перечисленными файлами-изображениями обучающего набора данных и метки класса
        (для режима "train") или без метки (для режима "inference"), .txt формат.
    model_dir : путь к файлу для сохранения или загрузки модели, .sav формат.
    save_inf: путь к файлу для сохранения результата предскаания модели, .txt формат.
